...
Mirrors the checker plugin pattern exactly: each fixable rule ID registers a `Remediator` (`ID() string`, `Remediate(ctx, client, repo, rule, result) (*Fix, error)`) via `remediation.Register(...)` in an `init()`, and `remediation.Run` (called from `runScan` after output write, before Issues/Slack) dispatches failing results to the matching one and opens/updates a PR. Gated at two levels, resolved by `config.EffectiveAutoRemediation`: `remediation.enabled` is the master switch (nothing runs if false), `auto_remediation` (per-rule, falling back to `remediation.auto_remediation`) decides which rules are in scope — a rule with no registered `Remediator` is silently skipped even if enabled.
